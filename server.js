...
app.post("/api/maths",requireUser,async(req,res)=>{
  try{
    ...
    if(action==="practice"){
      const data=await generateJson(client,
        `Create UK ${year} Key Stage 3 maths practice. Return ONLY valid JSON: {"title":"Practice title","questions":[{"question":"question text","marks":1,"hint":"short hint"}]}. Exactly 8 questions, starting accessible and becoming more challenging. Mix fluency, reasoning and problem-solving. Use marks from 1 to 3. Do not include answers or solutions. Use British maths terminology and age-appropriate numbers. No markdown.`,
        `Create practice questions for: ${topic}.`);
      return res.json({type:"maths-practice",year,topic,...data});
    }
...
const help={learn:'A guided lesson with worked examples and quick checks.',practice:'Eight questions with hints, then AI marking and worked answers.',test:'A timed 30-minute exam-style paper. No hints until you submit.'};
document.querySelectorAll('.maths-action').forEach(b=>b.onclick=()=>{document.querySelectorAll('.maths-action').forEach(x=>x.classList.remove('active'));b.classList.add('active');mathsMode=b.dataset.mathsmode;document.getElementById('mathsModeHelp').textContent=help[mathsMode];document.getElementById('mathsStart').textContent=mathsMode==='learn'?'Start Learn':mathsMode==='practice'?'Start Practice':'Start Real Test';document.getElementById('mathsResult').innerHTML='';currentPaper=null;});
...
document.getElementById('mathsStart').onclick=async function(){const h=document.getElementById('mathsResult');h.innerHTML='<div class="maths-loading"><div class="spinner"></div>Preparing '+safe(mathsMode)+' for '+safe(selectedTopic)+'...</div>';this.disabled=true;try{const d=await mathsFetch({action:mathsMode,topic:selectedTopic});if(mathsMode==='learn')renderLearn(d);else renderPaper(d,mathsMode==='test');}catch(e){h.innerHTML='<div class="error">'+safe(e.message)+'</div>';}finally{this.disabled=false;}};
})();</script>`;
...
app.get("*",sendApp);
app.listen(PORT,"0.0.0.0",()=>console.log(`AI Learning Switchboard v2.5.0 running on port ${PORT}`));