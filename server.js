...
const vocabAiHits=new Map();
app.post("/api/vocabulary/sentence-feedback",async(req,res)=>{
  const ip=String(req.ip||req.socket?.remoteAddress||"unknown"), now=Date.now(), recent=(vocabAiHits.get(ip)||[]).filter(t=>now-t<3600000); if(recent.length>=60) return res.status(429).json({error:"AI Teacher has had lots of requests. Please try again shortly."}); recent.push(now); vocabAiHits.set(ip,recent);
  try{
    const supplied=String(req.headers["x-openai-key"]||"").trim(); const serverKey=String(process.env.OPENAI_API_KEY||"").trim(); const apiKey=supplied||serverKey; if(!apiKey) return res.status(503).json({error:"AI Teacher is temporarily unavailable."}); const client=new OpenAI({apiKey});
    ...
    if(sentenceWords.length<3) return res.status(400).json({error:"Write a complete sentence with at least 3 words before asking the AI Teacher."});
    const data=await generateJson(client,
      'You are a warm, encouraging UK Year 7 English teacher. Review one pupil sentence using a target vocabulary word. FIRST and most importantly judge whether the sentence demonstrates the CORRECT MEANING of the target vocabulary word. Merely containing the target word is NEVER enough. A grammatically complete sentence with meaningless, nonsensical, circular, question-like misuse, or semantically incorrect use MUST have correct=false. Only after meaning is valid should you check grammar, spelling, punctuation and clarity. Do not be harsh. Return ONLY valid JSON: {"correct":true,"praise":"one short positive comment","verdict":"Correct or Needs improvement","why":"2-4 clear sentences explaining exactly WHY the usage is correct or wrong, referring to the target word meaning and the pupil sentence","feedback":"one or two clear age-appropriate improvement points","correctedSentence":"a corrected natural version of the pupil sentence","wordUse":"brief explanation of the target word meaning in this context"}. Always explain the reasoning: if correct, say what the word means and why it fits this sentence; if incorrect, teach the pupil step by step: explain the word meaning, identify exactly what is wrong in their sentence, show how to form a correct sentence, explicitly teach capital letters and the correct end punctuation (. ? !), then give a corrected model sentence. Encourage the pupil to try again in their own words. Also distinguish vocabulary usage from spelling, grammar and punctuation. Preserve the pupil meaning. If already excellent, correctedSentence should repeat it with only necessary corrections. No markdown.',
