...
function schoolInfo(dateStr){const p=dateStr.split('-').map(Number),d=new Date(p[0],p[1]-1,p[2]);d.setHours(0,0,0,0);const days=Math.floor((d-WEEK1)/86400000),week=Math.floor(days/7)+1,dow=(d.getDay()+6)%7;if(week<1||week>10||dow>4)return null;return{week,words:SCHOOL_WEEKS[week].slice(dow*2,dow*2+2)};}
...
html=html.replace("const r=await fetch('/api/english',{method:'POST'", "const endpoint=body.action==='daily-vocab'&&Array.isArray(body.schoolWords)&&body.schoolWords.length===2?'/api/school-vocab':'/api/english';const r=await fetch(endpoint,{method:'POST'");
